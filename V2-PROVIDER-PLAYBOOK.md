...
| 22 | Entra ID (azuread) | pushed | good dogfood candidate (live tenant). 9 config-core TF types (group + application_registration + service_principal + named_location + conditional_access_policy + administrative_unit + directory_role_assignment + group_member + app_role_assignment); spec at docs/v2-specs/azuread.md. Provider hashicorp/azuread ~> 3.x, over Microsoft Graph. Auth: OAuth2 client-credentials FORM exchange to login.microsoftonline.com/<tenant>/oauth2/v2.0/token (scope .default) → short-lived Bearer refreshed on mid-run 401 (Keycloak analogue); env ARM_TENANT_ID/ARM_CLIENT_ID/ARM_CLIENT_SECRET; providers.tf keyless. OData `{"value":[...],"@odata.nextLink":"<abs-url>"}` envelope; pager FOLLOWS the server-supplied absolute nextLink ONLY after isGraphURL host-validates it stays https on graph.microsoft.com (token-exfil guard, off-host refusal tested + page-1 items preserved); redirect-refusing client. DEFINING HAZARD: v3 changed EVERY object import from bare UUID to a Graph-PATH prefix — /groups/<id>, /applications/<id>, /servicePrincipals/<id>, /identity/conditionalAccess/{policies,namedLocations}/<id>, /directory/administrativeUnits/<id>; composites `<gid>/member/<mid>` (NO leading slash) + `/servicePrincipals/<sp>/appRoleAssignedTo/<id>`; directory_role_assignment BARE opaque id. Uses object `id` not `appId`. Precomputed + EscapeHCLTemplate'd, Properties["importID"]. 7 top-level roots + 2 relationship fan-outs (per-group /members, per-SP /appRoleAssignedTo; gid/spid PathEscaped). Reviewed (2 agents): security APPROVE (nextLink guard fail-closed, secret form-body-only, Bearer header-only, no cred fields decoded/enumerated); correctness REQUEST-CHANGES→fixed: MED Capabilities.IAM true→false (mirror keycloak; Entra access-control is ordinary resources), LOW top-level-403 now counts toward systemic guard (no-permissions app → actionable error not silent-empty), LOW PathEscape fan-out ids. directory_role (unimportable) + users (PII/scale) + credential resources + /beta deferred; per-SP first-party-SP filter + $select/$top + app/SP passwordCredentials/keyCredentials Phase-B (Graph returns cred METADATA not values on read) |
| 23 | Heroku | todo | |
...
- **Next up:** Batch 3 — Heroku (#23), then Octopus Deploy, commercetools, Opal. (Entra ID/azuread #22 pushed. 22/40 built.)
