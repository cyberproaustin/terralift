...
var awsTypeToTFExtra = map[string]string{
	...
	// dynamodb:table is in the core map (aws_dynamodb_table); the dead aws_dynamodb_global_table
	// shadow here was wrong (a plain table is not a global table) — removed.
	...
	// elasticloadbalancing:* live in the core map (types.go) — the loadbalancer
	// entry here previously shadow-conflicted (aws_elb vs the core aws_lb) and was
	// dead; removed to avoid a landmine if map priority ever changes.
	...
	// iam:role is in the core map (aws_iam_role); the dead aws_iam_service_linked_role
	// shadow here was wrong (a role is not a service-linked role) — removed.
