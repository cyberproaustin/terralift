// Package vultr implements the CloudProvider contract for Vultr — a flat, token-scoped
// IaaS provider. Enumeration is via the Vultr REST API v2 (no CLI, Bearer key); export
// reuses `terraform plan -generate-config-out`.
//
// NOTE: Phase-A scaffold. Curation (docs/v2-specs/vultr.md) is added when live-validated.
package vultr
...
func (p *Provider) Templates() provider.ProviderTemplates {
  ...
  backend "s3" {
    ...
    # Any supported backend works; Vultr has no native state store.
  }
}
`,
		Pipeline: `# GitHub Actions: plan-on-PR + gated apply. Auth via a VULTR_API_KEY secret,
...
jobs:
  plan:
    ...
    env: { VULTR_API_KEY: '${{ secrets.TF_VULTR_API_KEY }}' }
    steps:
